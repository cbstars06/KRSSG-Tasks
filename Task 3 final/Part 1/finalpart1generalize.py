...
imgGoal = cv.bitwise_and(img,img,mask=mask_start)
...
mask = mask0+mask1
imgStart = cv.bitwise_and(img,img,mask=mask)

#Black Border
border = np.zeros((rows,columns), dtype=np.uint8)
cv.rectangle(border,(7,7),(columns-7,rows-7), (255,255,255) ,thickness = -1)

#Obstacle Image
mask_obs = cv.bitwise_not(mask_start+mask)
imgObstacle = cv.bitwise_and(img,img,mask=mask_obs)
imgObstacle = cv.bitwise_and(imgObstacle,imgObstacle,mask=border)
...
start_node = Centre(contours_start[start_num-1])
goal_node = Centre(contours_goal[goal_num-1])
...
class RRTStarConnect:
    def __init__(self,start_node,goal_node,img,contours_obstacle,rows,columns):
        ...
        # self.contours_start = contours_start
        # self.contours_goal = contours_goal
    ...
    def plotPath(self):
        ...
        cv.line(self.img,bestNode,startNode,(0,0,255),thickness=2)
        cv.line(self.img,bestNode,goalNode,(0,0,255),thickness=2)
        nodes = self.start[:,0]
        while(startNode != self.start_node):
            for i in range(len(nodes)):
                ...
                if(nodes[i]==startNode):
                    b=i
                    break
            cv.line(self.img,self.start[b,0],self.start[b,1],(0,0,255),thickness=2)
        ...
        while(goalNode != self.goal_node):
            for i in range(len(nodes)):
                ...
                if(nodes[i]==goalNode):
                    b=i
                    break
            cv.line(self.img,self.goal[b,0],self.goal[b,1],(0,0,255),thickness=2)
