...
class Image:
    ...
    def imageHandling(self):
        ...
        self.imgObstacle = cv.bitwise_and(self.img,self.img,mask=mask_obs)

        #Start Image
        lower1 = np.array([8,0,0])
        higher1 = np.array([100,255,255])
        mask_start = cv.inRange(imgHSV,lower1,higher1)
        self.imgStart = cv.bitwise_and(self.img,self.img,mask=mask_start)
        ...
        mask = mask0+mask1
        self.imgGoal = cv.bitwise_and(self.img,self.img,mask=mask)
    ...
    def update(self):
        ...
        self.start_node = self.Centre(self.imgStart)
        self.goal_node = self.Centre(self.imgGoal)
...
img =cv.imread('task3.1.png')
myImage = Image(img)
myImage.update()

rows,columns,ch = img.shape
       
path = RRTStarConnect(myImage.start_node,myImage.goal_node,img,myImage.contours_obstacle,rows,columns)
